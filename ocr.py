...
if __name__ == "__main__":
    img_path = "imgs/table-pic-for-ocr.png"
    ...
    for i, line in enumerate(results):
        ...
        blank_draw.text(xy=(float(x), float(y)), text=txt, font=font)
    ...
    for index, result in enumerate(results):
        output.append([str(result[0]), result[1], result[2]])

    output = json.dumps(output, ensure_ascii=False, indent=4)  # ensure_ascii：使用中文保存，缩进为4个空格

    with open('ocr_output/ocr.json', 'w+') as f:
        f.write(output)
